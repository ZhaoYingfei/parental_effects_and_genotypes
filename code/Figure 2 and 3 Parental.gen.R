...
data_01gen <- read_excel("2022herbivory_data241212.xlsx", sheet = "1st.gen")
str(data_01gen)

data_01gen <- data_01gen %>%
# Remove rows where 'no.leaves' contains NA values
filter(!is.na(no.leaves)) %>%
# Convert all columns from 'stolon.length' to 'sla' to numeric type
mutate(across(stolon.length:sla, ~as.numeric(.)))
...
#  3 stolon.mass          0.91   0     Shapiro-Wilk normality test
...
#  8 stolon.length        0.98   0.097 Shapiro-Wilk normality test
...
figure_2_vars <- c("total.mass", "total.leaf.mass", "petiole.mass", "stolon.mass", "root.mass", "ratio")
figure_3_vars <- c("no.ramets", "stolon.length", "total.leaf.area",
                     "no.leaves", "mla", "sla")
